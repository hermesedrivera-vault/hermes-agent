...
def _trust_gate_check(server_name: str, tool_name: str, tool_args: Optional[dict] = None) -> Optional[str]:
    ...
    Step 52: routes through the canonical MCP authorization guard
    (session+task+subagent identity, server+tool+target-bound approval,
    cron-aware, fail-closed) rather than the older session-only,
    single-shared-bucket request_elicitation_consent() CLI/TUI prompt.
    """
    ...
    if decision.get("approved"):
        return None
    logger.info("MCP trust gate: call to '%s' on untrusted server '%s' denied: %s",
                tool_name, server_name, decision.get("message"))
    return tool_error(decision.get("message") or (f"The user did not approve running write-capable MCP tool "
                      f"'{tool_name}' on untrusted server '{server_name}'. The command was NOT run. Do not "
