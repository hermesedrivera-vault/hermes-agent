...
def _guarded_action(task_id: Optional[str], action: str, command: str, args: list, ok: Dict[str, Any], err: str) -> str:
    """Input action on the task's current page, refused when the SSRF guard flags the page or the
    outbound-comm intent gate denies it (clicks/keypresses can submit forms / trigger sends)."""
    effective_task_id = _last_session_key(task_id or "default")
...
registry.register(
    ...
    check_fn=_install.check_browser_requirements,
    emoji="🖼️",
)
registry.register(
    name="browser_vision",
    ...
    check_fn=_install.check_browser_vision_requirements,
    emoji="👁️",
)
registry.register(
    name="browser_console",
