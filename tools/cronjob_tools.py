...
def _action_create(a: Dict[str, Any]) -> str:
    ...
    _deliver_decision = check_cron_deliver_change_guard(
        job_id=None,
        old_deliver=None,
        # Gate against the PRE-resolution value the caller actually typed,
        # not the resolved one. In a cron-context create,
        # _resolve_cron_context_deliver rewrites 'origin' (or an omitted
        # deliver) into the CREATING job's own concrete platform:chat_id —
        # that's the same target the creating run was already authorized
        # to reach, not a new model-chosen recipient, and must not be
        # re-gated just because it now looks like an explicit target after
        # resolution.
        new_deliver=deliver,
        is_create=True,
    )
...
def _update_core_fields(job: Dict[str, Any], a: Dict[str, Any], updates: Dict[str, Any]) -> Optional[str]:
    ...
    if deliver is not None:
        ...
        _deliver_decision = check_cron_deliver_change_guard(
            ...
            # Gate against the pre-resolution value actually typed, not the
            # cron-context-resolved concrete target (see _action_create's
            # identical comment for why: resolving 'origin' to the caller's
            # own platform:chat_id is not a new model-chosen recipient).
            new_deliver=_normalized_input_new_deliver,
            is_create=False,
        )
        if not _deliver_decision.get("approved", False):
            return (_deliver_decision.get("message")
                    or "BLOCKED: cron delivery-target authorization denied. Update NOT applied.")
        updates["deliver"] = _resolve_cron_context_deliver(_normalized_input_new_deliver)
