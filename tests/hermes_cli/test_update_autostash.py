...
def _make_update_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        if "tag" in joined and len(cmd) >= 3 and cmd[-3] == "tag":
            # STEP 70 G.1: _create_pre_reset_safety_tag(git_cmd, cwd, sha) creates
            # ``tag <name> <sha>``, then verifies via ``rev-parse <name>``. Track
            # the mapping so the verification step below succeeds by default —
            # every orphan/reset test exercises this safety-tag gate first.
            created_tags[cmd[-2]] = cmd[-1]
            return SimpleNamespace(stdout="", stderr="", returncode=0)
        if "rev-parse" in joined and cmd[-1] in created_tags:
            return SimpleNamespace(stdout=f"{created_tags[cmd[-1]]}\n", stderr="", returncode=0)
