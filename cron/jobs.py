...
def _record_run_outcome(
    ...
    try:
        ...
        if _store:
            _sid = f"cron:{job.get('id', '')}"
            _store.mint(
                claim_id=f"cron_execution.{job.get('id', '')}.{now}",
                source_uri=f"cron://{job.get('id', '')}",
                content={"job_id": job.get("id"), "success": success,
                         "error": error, "status": status},
                session_id=_sid,
                tool_name="cron_execution",
                ttl_seconds=300,
                facts={"job_id": job.get("id"), "success": bool(success)},
            )
            _store.mint(
                claim_id=f"cron_delivery.{job.get('id', '')}.{now}",
                source_uri=f"cron://{job.get('id', '')}",
                content={"job_id": job.get("id"),
...
# NOTE: clear_drift_alerted itself lives above (this fork kept the monolith
# instead of adopting the decomposition) — no re-export needed for it here.
# ---- END PLUGIN-COMPAT ----
