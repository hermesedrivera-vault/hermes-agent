        ...
        try:
            ...
            if pull_result.returncode != 0:
                ...
                if _cur_branch and _cur_branch != branch:
                    ...
                    # force-push/rebase.  Since local changes are already
                    # stashed, reset to match the remote exactly — but not
                    # blindly: STEP 70 G.3 verifies origin actually contains
                    # this branch's tip ancestry first. A local-only commit
                    # here (e.g. a manual fix, or work never pushed) would
                    # otherwise be silently destroyed by the reset below —
                    # the exact failure class that wiped local security work
                    # in the past. Fail closed on ANY uncertainty: an
                    # unverifiable check is never treated as "no divergence".
                    print(
