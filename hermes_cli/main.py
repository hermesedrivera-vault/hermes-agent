...
def _finalize_update_receipt(code: int, reason: str) -> None:
    ...
    Always followed by the fork-safety integrity line (2026-09-06, Ed's core complaint:
    the safety check existed but only Hermes checking manually surfaced it) — never
    something the caller has to remember to print separately.
    """
    try:
